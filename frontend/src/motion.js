...
export class MotionController {
  ...
  _sway() {
    ...
    this._addRot("spine", "y", n1 * SWAY_SPINE * a);
    this._addRot("chest", "y", n1 * SWAY_SPINE * 0.4 * a);
    this._addRot("head", "z", n2 * SWAY_HEAD * 0.6 * a);
    this._addRot("head", "x", n2 * SWAY_HEAD * 0.45 * a);
    this._addRot("head", "y", n1 * SWAY_HEAD * 0.3 * a);

    // shoulders ride the raw level (peaks visibly lift them)
    const lvl = this._speechLevel();
    this._addRot("leftShoulder", "z", lvl * SHOULDER_TALK);
    this._addRot("rightShoulder", "z", -lvl * SHOULDER_TALK);

    // hands talk
    this._addRot("leftUpperArm", "z", -Math.abs(n2) * HANDS * 0.3 * a);
    this._addRot("rightUpperArm", "z", Math.abs(n2) * HANDS * 0.3 * a);
    this._addRot("leftLowerArm", "z", n1 * HANDS * 0.7 * a);
    this._addRot("rightLowerArm", "z", -n1 * HANDS * 0.7 * a);
    this._addRot("leftHand", "z", n2 * HANDS * a);
    this._addRot("rightHand", "z", -n2 * HANDS * a);
    this._addRot("leftHand", "x", n1 * HANDS * 0.5 * a);
    this._addRot("rightHand", "x", n1 * HANDS * 0.5 * a);
  }
