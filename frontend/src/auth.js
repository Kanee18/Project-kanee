/**
 * Firebase auth + the user's beta-access record (companion-app copy — identical
 * to the landing site's, so both apps share one Firebase project/config).
 *
 * Account model (Firestore):
 *   users/{uid} = { email, displayName, betaAccess: bool, plan, createdAt }
 *
 * Beta access is granted MANUALLY in the Firestore console. Security rules
 * forbid the client writing `betaAccess`, so the gate can't be bypassed.
 */
